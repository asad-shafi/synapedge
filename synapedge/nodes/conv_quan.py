...
def _write_convolution_function(buffer: StringIO, func_name: str, inputs: List[str], outputs: List[str], attrs: Dict[str, Any], tensor_shape: Dict[str, Any]) -> None:
    ...
    min_inputs = 8  # Input and kernel
    has_bias = len(inputs) >= 9 # if bias present
    ...
    try:
        input_dims = tensor_shape[x_name]
        kernel_dims = tensor_shape[w_name]
        output_dims = tensor_shape[y_name]
    ...
    if not multi_dimensional:
        ...
        buffer.write(f"    float* input = (float *){x_name};\n")
        buffer.write(f"    float* kernel = (float *){w_name};\n")
        buffer.write(f"    float* output = (float *){y_name};\n")
        if has_bias:
        ...
        if K_h == 1 and K_w == 1:
            ...
            if strides[0] == 1 and strides[1] == 1 and pads == [0, 0, 0, 0] and dilations == [1, 1]:
                ...
                buffer.write("                int32_t ker_val = kernel[k * C_in + c];  // 1x1 kernel\n")
     ...
     # Using multi-dimensional indexing, we do not need to declare flat pointers.
        if K_h == 1 and K_w == 1:
            ...
            if strides[0] == 1 and strides[1] == 1 and pads == [0, 0, 0, 0] and dilations == [1, 1]:
                ...
                buffer.write("        for (int c = 0; c < C_in; ++c) {\n")
                buffer.write("            for (int k = 0; k < K; ++k) {\n")
                buffer.write(f"                int32_t b= {bias_name}[k];\n")
                buffer.write(f"                int32_t ker_val = {w_name}[k][c][0][0];  // 1x1 kernel\n")
                buffer.write("                for (int h = 0; h < H_out; ++h) {\n")
                buffer.write("                    for (int w = 0; w < W_out; ++w) {\n")
                buffer.write("                        /* Accumulate directly into the output */\n")
                if has_bias:
                    buffer.write(f"                    if (c == 0) {y_name}[n][k][h][w] = b;\n")
                buffer.write(f"                        {y_name}[n][k][h][w] += {y_name}[n][c][h][w] * ker_val;\n")
                buffer.write("                    }\n")
                buffer.write("                }\n")
                buffer.write("            }\n")
                buffer.write("        }\n")
                ...
                buffer.write("                        int w_in = w * stride_w - pad_l;\n")
                buffer.write("                        if (h_in >= 0 && h_in < H_in && w_in >= 0 && w_in < W_in) {\n")
                buffer.write(f"                            sum += {x_name}[n][c][h_in][w_in] * {w_name}[k][c][0][0];\n")
                buffer.write("                        }\n")
                buffer.write("                    }\n")
                if has_bias:
                    buffer.write(f"                    sum += {bias_name}[k];\n")
                buffer.write(f"                    {y_name}[n][k][h][w] = sum;\n")
                buffer.write("                }\n")
                buffer.write("            }\n")
                buffer.write("        }\n")
                buffer.write("    }\n")
            ...
            buffer.write(f"                                sum += {x_name}[n][c][h_in][w_in] * {w_name}[k][c][kh][kw];\n")
            buffer.write("                            }\n")
            buffer.write("                        }\n")
            buffer.write("                    }\n")
            buffer.write(f"                    {y_name}[n][k][h][w] = sum;\n")
