...
def _write_quantizelinear_function(functions: Dict[str, Any],buffer: StringIO, func_name: str, inputs: List[str], outputs: List[str], attrs: Dict[str, Any], tensor_shape: Dict[str, Any]) -> None:
    """Generates C code for QuantizeLinear operator that works for both uint8_t and int8_t."""
    x_name = inputs[0]
   ...
   # Calculate output dimensions if not provided
    if not y_shape:
        ...
        for i in range(len(functions)):  # Iterate using index
            ...
            if helperfunc._sanitize_name(func['name']) == func_name: # Update output shape of the producer node (or curent node)
                ...
                functions[i]['intermediate_tensors_shape'][f"{outputs[0]}_dtype"] = "uint8_t"#tensor_shape.get(x_name+"_dtype", "uint8_t")
    ...
    # Write min/max constants based on output type
    if out_dtype == "uint8_t":
        buffer.write("    const int32_t min_val = 0;\n")
        buffer.write("    const int32_t max_val = 255;\n\n")
    else:  # int8_t
        buffer.write("    const int32_t min_val = -128;\n")
        buffer.write("    const int32_t max_val = 127;\n\n")
    
    buffer.write(f"float inv = 1.0f/{y_scale_name}[0];\n")
    ...
    if total_scale_elements == 1:
        ...
        buffer.write(indent + f"float scaled = {x_name}{idx_str} * inv;\n")#/ {y_scale_name}[0];\n")
        buffer.write(indent + f"float rounded = roundf(scaled);\n")  # Using roundf instead of rintf for better portability
        buffer.write(indent + f"float result = rounded + (float){y_zp_name}[0];\n")
        buffer.write(indent + f"int32_t quantized = (int32_t)result;\n")
        buffer.write(indent + "quantized = quantized < min_val ? min_val : quantized;\n")
        buffer.write(indent + "quantized = quantized > max_val ? max_val : quantized;\n")
        buffer.write(indent + f"{y_name}{idx_str} = ({out_dtype})quantized;\n")
    ...
    # Per-axis quantization
        ...
        # Write quantization operation
        buffer.write(indent + f"float scaled = {x_name}{idx_str} / {y_scale_name}[{axis_idx}];\n")
        buffer.write(indent + f"float rounded = roundf(scaled);\n")
        buffer.write(indent + f"float result = rounded + (float){y_zp_name}[{axis_idx}];\n")
        buffer.write(indent + f"int32_t quantized = (int32_t)result;\n")
        buffer.write(indent + "quantized = quantized < min_val ? min_val : quantized;\n")
        buffer.write(indent + "quantized = quantized > max_val ? max_val : quantized;\n")
        buffer.write(indent + f"{y_name}{idx_str} = ({out_dtype})quantized;\n")
